...
//
// Reuses Scratch's own camera feed (runtime.ioDevices.video) instead of
// opening a second independent camera stream, since many webcams/drivers
// refuse a second concurrent getUserMedia() request ("Could not start
// video source").
...
(function (Scratch) {
  ...
  async function getVideoFrame() {
    const vm = Scratch.vm;
    const videoDevice = vm.runtime.ioDevices.video;

    // Turn the camera on if it isn't already (safe to call even if
    // Scratch's own Video Sensing extension already turned it on).
    await videoDevice.enableVideo();

    // The frame may not be available on the very first call(s) while the
    // camera is still starting up, so retry briefly.
    for (let attempt = 0; attempt < 30; attempt++) {
      const frame = videoDevice.getFrame({ format: 'canvas' });
      if (frame) return frame;
      await new Promise((resolve) => setTimeout(resolve, 100));
    }
    throw new Error('Camera did not produce a frame in time');
  }
